...
function buildDiagnosticReportId(requestId) {
    return `diag_${requestId}`;
}
function detectPrimaryReason(gateResult) {
    const reasonCodes = gateResult.decisionEnvelope.trace.reasonCodes;
    return reasonCodes[0] ?? "UNKNOWN_REASON";
}
function deriveSeverity(gateResult, replayResult, evidenceBundle) {
...
function buildOperatorExplanation(gateResult) {
    const primaryReason = detectPrimaryReason(gateResult);
...
function buildInvariantChecks(request, gateResult) {
        ...
        },
        {
            ...
            status: !checked.has("INV-02")
                ...
                : request.provenanceRef === null ||
                    ...
                    ? "FAIL"
                    : "PASS",
            explanation: request.provenanceRef === null
        ...
        },
        {
            ...
            status: !checked.has("INV-03")
                ...
                : request.approvalRequired && request.approvalArtifact === null
                    ? "FAIL"
                    : "PASS",
            explanation: request.approvalRequired && request.approvalArtifact === null
        ...
        },
        {
            ...
            status: !checked.has("INV-08")
                ...
                : request.sensitive && (!request.controlStatus.criticalControlsValid || request.controlStatus.stale)
                    ? "FAIL"
                    : "PASS",
            explanation: request.sensitive && (!request.controlStatus.criticalControlsValid || request.controlStatus.stale)
        ...
        },
        {
            ...
            status: !checked.has("INV-11")
                ...
                : reasonCodes.has(REASON_CODES.MALFORMED_REQUEST) || reasonCodes.has(REASON_CODES.UNKNOWN_ACTION_CLASS)
                    ? "FAIL"
                    : "PASS",
            explanation: reasonCodes.has(REASON_CODES.MALFORMED_REQUEST)
...
function buildDecisionPath(request, gateResult, evidenceBundle) {
    const reasons = new Set(gateResult.decisionEnvelope.trace.reasonCodes);
    const inputFailed = reasons.has(REASON_CODES.MALFORMED_REQUEST) ||
        reasons.has(REASON_CODES.UNKNOWN_ACTION_CLASS);
    const policyOrProvenanceFailed = reasons.has(REASON_CODES.NO_POLICY_PACK) ||
        reasons.has(REASON_CODES.NO_PROVENANCE);
    const proposalBoundaryFailed = reasons.has(REASON_CODES.AI_CANNOT_AUTHORIZE) ||
        reasons.has(REASON_CODES.INVALID_PROPOSAL);
    const approvalFailed = reasons.has(REASON_CODES.REQUIRED_APPROVAL_MISSING) ||
    ...
    const controlTrustFailed = reasons.has(REASON_CODES.CONTROL_STALE_OR_INVALID) ||
...
function buildRecommendedAction(gateResult, replayResult) {
    if (!replayResult.matchedOriginalOutcome) {
        return {
...
export class DiagnosticReportService {
    createReport(args) {
        const { request, gateResult, evidenceBundle, replayResult, exportManifest = null, chainVerified } = args;
        const primaryReason = detectPrimaryReason(gateResult);
