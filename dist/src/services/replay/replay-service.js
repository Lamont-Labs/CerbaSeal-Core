...
export class ReplayService {
    ...
    constructor(executionGateService) {
        this.executionGateService = executionGateService;
    }
    replay(bundle) {
        const replayed = this.executionGateService.evaluate(bundle.request);
        const matchedOriginalOutcome = replayed.decisionEnvelope.finalState === bundle.decisionEnvelope.finalState &&
    ...
    exportReferencesOriginalEvidence(bundle, manifest) {
        ...
        const hashesMatch = manifest.sourceEventHashes.every((hash, index) => hash === bundle.eventChain[index]?.entryHash);
        return (manifest.evidenceBundleId === bundle.evidenceBundleId &&
