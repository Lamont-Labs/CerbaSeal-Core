...
export function runSystemHealthCheck() {
    ...
    const checks = [];
    checks.push(runCheck("reject_scenario_evaluates_to_reject", () => {
    ...
    checks.push(runCheck("hold_scenario_evaluates_to_hold", () => {
    ...
    checks.push(runCheck("allow_scenario_evaluates_to_allow", () => {
    ...
    checks.push(runCheck("operator_action_report_reject", () => {
        const { gateResult } = buildSupportRejectScenario();
        const report = createOperatorActionReport(gateResult);
        if (!report.recommendedAction)
            throw new Error("No recommendedAction");
        return `severity=${report.severity}`;
    }));
    checks.push(runCheck("operator_action_report_hold", () => {
        const { gateResult } = buildSupportHoldScenario();
        const report = createOperatorActionReport(gateResult);
        if (!report.recommendedAction)
            throw new Error("No recommendedAction");
    ...
    checks.push(runCheck("operator_action_report_allow", () => {
        const { gateResult } = buildSupportAllowScenario();
        const report = createOperatorActionReport(gateResult);
        if (!report.recommendedAction)
            throw new Error("No recommendedAction");
    ...
    checks.push(runCheck("evidence_bundle_reject", () => {
        ...
        if (!bundle.evidenceBundleId)
            throw new Error("No evidenceBundleId");
        return `evidenceBundleId=${bundle.evidenceBundleId}`;
    }));
    checks.push(runCheck("evidence_bundle_allow", () => {
        const logService = new AppendOnlyLogService();
        const evidenceService = new EvidenceBundleService(logService);
    ...
    checks.push(runCheck("audit_chain_verifies", () => {
        const logService = new AppendOnlyLogService();
        const evidenceService = new EvidenceBundleService(logService);
        const { request, gateResult } = buildSupportAllowScenario();
