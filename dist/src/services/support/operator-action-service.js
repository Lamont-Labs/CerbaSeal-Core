...
function getPrimaryReasonCode(gateResult) {
    const codes = gateResult.decisionEnvelope.trace.reasonCodes;
    const causeCodes = codes.filter((c) => c !== REASON_CODES.DECISION_ALLOWED &&
        c !== REASON_CODES.DECISION_HELD &&
        c !== REASON_CODES.DECISION_REJECTED);
    return causeCodes[0] ?? null;
}
export function createOperatorActionReport(gateResult) {
    ...
    if (reasonCodes.includes(REASON_CODES.INVALID_APPROVAL_AUTHORITY) ||
        ...
        return {
            ...
            rationale: "The approval artifact is missing, mismatched, or does not satisfy required authority.",
            severity: "CRITICAL",
            operatorChecklist: [
    ...
    if (reasonCodes.includes(REASON_CODES.MALFORMED_REQUEST) ||
