...
function baseApproval(requestId, overrides = {}) {
    return {
        approvalId: overrides.approvalId ?? `approval_${requestId}`,
        approverId: overrides.approverId ?? "compliance_officer_001",
        forRequestId: requestId,
        approverAuthorityClass: overrides.approverAuthorityClass ?? "compliance_officer",
...
describe("Case 4 — Repeated approvals across unrelated actions", () => {
    ...
    const requests = requestIds.map(id => baseRequest(id, {
        approvalArtifact: {
            ...baseApproval(id),
